...
int _tmain(int argc)
{
	...
	int nImgNum = nLine / 2;
	Mat data_mat(nImgNum, 324, CV_32FC1,Scalar(255)), res_mat(nImgNum, 1, CV_32FC1, Scalar(255));
	Mat src, trainImg(Size(28, 28), 8, 3);
	...
	for (string::size_type i = 0; i != img_path.size(); i++)
	{
		...
		resize(src, trainImg,Size(28,28));
		...
		{
			//cvmSet(data_mat, i, n, *iter);//存储HOG特征
			data_mat.at<float>(28 * n + i) = *iter;
			
			n++;
		}
		//cvmSet(res_mat, i, 0, img_catg[i]);
		data_mat.at<float>(i) = img_catg[i];
